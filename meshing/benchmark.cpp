...
struct Params {
  ...
  double d = 2.0;        // offset distance from the big's center
...
static void checkParams() {
  ...
  if (ratio_ab * P.big < P.small) {
    std::cerr << "Error: ratio_ab * big (" << ratio_ab * P.big << ") < small ("
              << P.small << ")\n";
    std::exit(EXIT_FAILURE);
  }
  if (small_outer > P.big) {
    std::cerr << "Error: small_outer (" << small_outer << ") > big (" << P.big
              << ")\n";
    std::exit(EXIT_FAILURE);
  }
}
...
static double meshSizeCallback(int, int, double x, double y, double z,
  ...
  const double ratio_ab = (b > eps) ? (a / b) : 0.0; 
  const double small_outer = (a > eps) ? (P.small * (b / a)) : P.small;  
