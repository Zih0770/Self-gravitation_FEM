// mpirun -np 4 bin/ex2 -m data/prem.msh -o 2 -mth 1 -deg 16
#include "common.hpp"
#include "mfem.hpp"
...
class RadialInterpCoefficient : public Coefficient {
 ...
 public:
  RadialInterpCoefficient(std::vector<real_t> r_dim,
                          std::vector<real_t> rho_dim, real_t L)
...
static bool ReadPREMAndSplitLayers(
    const std::string &filename, std::vector<std::vector<real_t>> &layers_r_dim,
    std::vector<std::vector<real_t>> &layers_rho_dim, real_t radius_scale = 1.0,
    ...
    if (std::abs(R[i] - R[i - 1]) <= tol) {
      if (!curR.empty()) {
        ...
        for (size_t k = 0; k < curR.size(); ++k) {
          if (k == 0 || curR[k] > fR.back()) {
            fR.push_back(curR[k]);
            fRH.push_back(curRH[k]);
          } else if (curR[k] == fR.back()) {
            fRH.back() = curRH[k];
          }
        }
...
int main(int argc, char *argv[]) {
  ...
  const char *mesh_file = "data/prem.msh";
  ...
  real_t L_scale = 6371e3;
  real_t RHO_scale = 5e3;
  ...
  if (!ReadPREMAndSplitLayers(property_file, layers_r_dim, layers_rho_dim)) {
    std::cerr << "Error: failed to read PREM property file: " << property_file
              << std::endl;
    return 1;
  }
  ...
  auto bdr_marker = ExternalBoundaryMarker(&pmesh);
  ...
  // Interpolate the solution along a radius
  {
    SolutionRadialInterpolator ri(pmesh, mfem::Ordering::byNODES, 10.0);

    const int n = 1000;
    const double r0 = 0.0, r1 = 1.2;

    ri.Interpolate(x, n, r0, r1);
    ri.Write("results/prem_phi_vs_r.dat");
  }
