  ...
  if (myid == 0)
{
    ...
    {
        const auto &r = layers_r_dim[i];
        if (!r.empty())
        {
            std::cout << "Layer " << i
                      << ": r_min = " << r.front() / L_scale
                      << ", r_max = " << r.back() / L_scale
                      << std::endl;
        }
        else
        {
            std::cout << "Layer " << i << ": empty" << std::endl;
        }
    }
}
  ...
  for (int i = 0; i < num_layers; ++i)
  {
      layer_coeffs.emplace_back(std::make_unique<RadialInterpCoefficient>(layers_r_dim[i], layers_rho_dim[i], L_scale));
  }
  ...
  {
      SolutionRadialInterpolator rho_interp(pmesh, mfem::Ordering::byNODES, 0.0);

      const int n = 1000;
      const double r0 = 0.546225, r1 = 0.878983;
      real_t eps = 1e-6;

      L2_FECollection rho_fec(order, dim);
      ParFiniteElementSpace rho_fes(&pmesh, &rho_fec);
      ParGridFunction rho_gf(&rho_fes);
      rho_gf.ProjectCoefficient(rho_coeff);
      rho_interp.Interpolate(rho_gf, n, r0+eps, r1);
      rho_interp.Write("results/prem_rho_vs_r.dat");
  }
...
/*
  ...
  if (method == 2) {
      if (linearised == 0) {
          ...
          auto c = PoissonLinearisedMultipoleOperator(MPI_COMM_WORLD, vfes.get(), &fes, rhs_coeff, lmax);
          c.Assemble();
          if (myid == 0) {
              end_time = MPI_Wtime();
              assembly_time = (end_time - start_time);
